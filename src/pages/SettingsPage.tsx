...
export const SettingsPage = ({ me, section, onBack, onLogout, uiVersion, onAvatarFile, onWallpaperFile, theme, onTheme, onResetTheme }: Props): JSX.Element => (
  <div className="flex h-full flex-col overflow-hidden rounded-2xl border border-white/20 p-4" style={{ backgroundColor: `rgba(255,255,255,${theme.panelOpacity})`, backdropFilter: `blur(${theme.contentBlur}px)` }}>
    ...
    <div className="flex-1 overflow-y-auto rounded-xl bg-white/5 p-4 pr-2">
      ...
      {section === 'personalization' && (
        <div className="space-y-4">
          ...
          <input className="accent-color-picker h-11 w-full rounded-2xl border border-white/20" type="color" value={theme.accentColor} onChange={(e) => onTheme('accentColor', e.target.value)} />
