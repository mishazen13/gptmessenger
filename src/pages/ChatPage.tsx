...
export const ChatPage = (props: Props): JSX.Element => {
  const {
    me, users, activeChat, getDisplayName, getAvatarUrl, onOpenFriendProfile, onContextMenu,
  ...
  const submitWithFx = (e: React.FormEvent<HTMLFormElement>): void => {
    if (messageText.trim()) {
      setFlyingText(messageText.trim().slice(0, 36));
      setTimeout(() => setFlyingText(''), 600);
    }
    onSend(e);
  };

  return (
    <div className="flex h-full flex-col rounded-2xl border border-white/20 p-4" style={{ backgroundColor: 'rgba(71,85,105,0.15)' }}>
        ...
        {isCallActive && callExpanded ? (
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              {peer ? <button className="w-full truncate text-left text-lg font-semibold hover:underline" onClick={() => onOpenFriendProfile(peer.id)} type="button">{getDisplayName(peer)}</button> : <h2 className="truncate text-lg font-semibold">{activeChat?.name ?? 'Чат'}</h2>}
        ...
        {activeChat?.messages.map((message) => {
          ...
          return (
            <article
              ...
              className="animate-fadeIn w-fit max-w-[85%] break-words whitespace-pre-wrap px-3 py-2 text-sm"
      ...
      {replyToMessageId && <div className="mb-2 flex items-center justify-between rounded-xl bg-white/10 px-3 py-2 text-xs"><span>Ответ</span><button className="text-cyan-400" onClick={onClearReply} type="button">Отмена</button></div>}

      <div className="relative mt-5 flex flex-col rounded-2xl border border-white/20 p-2" style={{ backgroundColor: 'rgba(71,85,105,0.15)' }}>
        {flyingText && <div className="pointer-events-none absolute bottom-12 left-5 rounded-xl bg-cyan-400/80 px-3 py-1 text-xs text-slate-950 animate-sendFly">{flyingText}</div>}
        ...
        {!!attachedFiles.length && <div className="mt-2 grid grid-cols-2 gap-2">{attachedFiles.map((f) => <button key={f.id} type="button" className="rounded bg-white/10 p-2 text-left text-xs" onClick={() => onRemoveAttachedFile(f.id)}>{f.name}</button>)}</div>}
      </div>
    </div>
  );
};
